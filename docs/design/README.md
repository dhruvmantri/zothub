...
Icon decided (A, stacked). Open: marks → production SVG (outline from licensed Instrument); the maintainer's pending component-level UI changes (land in the component-library phase). Before public launch: hero-photo licensing + WebP/AVIF optimization.

Next: **component library** (each part as a real stateful piece, incl. the maintainer's UI changes) → remaining screens → implementation (port tokens into `src/index.css`, self-host font) → archive pass.
