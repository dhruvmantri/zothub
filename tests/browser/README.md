...
| `ux17-empty-states.mjs` | `UX17a`/`UX17b`: the two discovery pages used to promise each other when both were empty, so a visitor went in a circle. Asserts the live club count actually renders (not a hard-coded number), that neither page claims the other has something, that the way out lands on `/clubs`, that a missing count never prints "0 clubs are here", and that the Clubs page no longer carries stage copy. |
| `ux24-follow-notification.mjs` | `UX24`: a notification from a followed club must be a LINK and must open the right surface. `new_post` does not say whether `related_id` is a role or an event — only the generated title does — so the destination, not the anchor tag, is the real assertion. |
