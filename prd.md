...
- 🟡 Hourly `send-reminders-hourly` cron job (event reminders, deadline reminders, new-post notifications, all with idempotency via `reminder_logs`) — scheduled **manually, out-of-repo**, so its existence is not provable from the repo; confirm live via the read-only `cron.job` SQL in `plan.md`'s Launch-ops checklist.
...
**Operational / trust**
- ~~**Nightly `archive_past_events()` job scheduling is unverified**~~ — 🟡 **Committed in-repo (WS6, 2026-07-13):** migration `20260713000100` schedules the nightly archive via `cron.schedule` (idempotent; verified end-to-end on a local pg_cron harness). Still pending: `supabase db push --linked` and a live `cron.job` check (which must also confirm the manually-scheduled `send-reminders-hourly`) — exact read-only SQL in `plan.md`'s Launch-ops checklist. *(plan.md WS6.)*
...
- [ ] `select * from cron.job;` confirms reminder + archive jobs are scheduled and idempotent *(the archive schedule is now committed in-repo — migration `20260713000100`, WS6 — but neither job is confirmed live from the repo: `send-reminders-hourly` was scheduled manually out-of-repo, and the archive job needs `db push` first; run the read-only SQL in `plan.md`'s Launch-ops checklist)*
