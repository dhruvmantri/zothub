...
- ~~**Clubs are not notified when a student applies**~~ — ✅ **Closed (WS1, 2026-07-10).** A new `AFTER INSERT` trigger on `applications` posts an in-app notification to the owning club, and a new `application_notification` email (recipient resolved server-side from the opportunity) is sent — both gated on the club's `application_updates` preference. Journey 1's "in-app + email notification per application" now holds.
...
3. **Receive applications**: in-app + email notification per application *(✅ implemented in WS1 — `on_new_application` trigger + `application_notification` email, gated on the club's `application_updates` preference)*; review — filter by opportunity, read correctly-labeled answers, download resumes, bulk accept/reject, export CSV.
