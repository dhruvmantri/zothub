...
> **🟡 Auth-orphan cleanup — code COMPLETE in repo (2026-07-13); pending the read-only production audit + `db push`.** Migrations `20260713000200` (deterministic row cleanup) + `20260713000300` (restore/add the 11 managed auth.users FKs; `messages` deliberately excluded, fails loud on orphaned profiles). **Before pushing: run `scripts/audit_auth_orphans.sql` against production and STOP if its Q4 (manual-review) returns any profile row.** Completion record below. The remaining backlog is **WS9 (future features)** — do not start net-new features while any launch-readiness item is open; the immediate remaining work is WS6's human-owned launch-ops ownership items plus this pass's production audit + push.
...
| `messages` — **both** parties dead | safe to delete — invisible to every living user (RLS: `sender_id = auth.uid() OR receiver_id = auth.uid()`) | DELETE (one-time) | **no FK** (see below) |
| `messages` — exactly **one** party dead | **preserve** — the living party's conversation history | none | **no FK** (see below) |
...
**`messages` — no FK is added (revised policy).** `sender_id`/`receiver_id` are `NOT NULL`, so `ON DELETE SET NULL` is impossible without a schema change; `ON DELETE CASCADE` (the original, restore-lost behavior) would delete a **living** user's conversation history when the other party is deleted — contradicting the preservation decision; `RESTRICT/NO ACTION` would block deleting any account that ever messaged. Preserving history while allowing account deletion needs a **separate product decision** (make the columns nullable + SET NULL, or a "deleted user" tombstone). Until then messages keep their current no-FK behavior; the cleanup only one-time-deletes both-parties-dead rows (unreadable by anyone). *Follow-up item — not part of this pass.* (On a healthy DB the original `messages` CASCADE FKs are still present; this migration neither adds nor removes them — it simply does not manage `messages`.)
...
**What shipped (migrations carry the actual project date 2026-07-13, so they sort before the applied WS8 07-14 migrations; logic is independent of WS8, so apply order is irrelevant and `db push` applies them as new versions):**
...
- **Migration `20260713000200_cleanup_orphaned_auth_user_refs.sql`** — deletes only the deterministic-junk classes; nulls `waitlist.reviewed_by`/`page_views.user_id` dead references; deletes both-parties-dead messages; **never touches** profiles, one-party-dead messages, or anything anchoring content. Idempotent; no-op on a healthy DB.
- **Migration `20260713000300_restore_auth_user_fks.sql`** — adds the **11 managed** auth.users FKs (9 CASCADE + 2 SET NULL) as **plain VALIDATED constraints** via a catalog-driven DO block. **Unambiguous success state:** it completes only if all 11 are present and validated. An existing FK is skipped **only** after verifying its **referenced column (`id`) and ON DELETE action** match the spec — a mismatch **raises**. If a managed column still has an orphan (only the manual-review profile classes), the `ADD CONSTRAINT` **fails loud** (raises with guidance) and the whole migration rolls back, rather than leaving an unvalidated constraint. Messages FKs and the pre-existing WS4/WS8 FKs are excluded. Idempotent.

**Verified (two local PG16.13 harnesses):**
- *Pristine path:* all **44** migrations apply cleanly in sorted order (the 07-13 cleanup runs before the WS8 07-14 migrations — no dependency inversion); both new migrations re-run idempotently; end state **15 auth FKs / 15 validated** (the extra 2 vs. production are the original `messages` CASCADE FKs, which a healthy DB never lost and this pass leaves untouched); audit script runs clean and read-only.
- *Production-drift simulation:* applied the 42 pre-existing migrations, **dropped the 7 original auth FKs** (as the restore did — messages then has no FK), seeded the known facts (8 orphaned + 3 valid `user_roles`) plus one orphan of every class **and** the preservation cases (orphaned club+student profiles anchoring an opportunity + 2 applications; one-party-dead messages both directions; dead reviewer on a valid waitlist row; dead viewer + anonymous page_views). Audit reported **exactly** the seeded matrix (incl. trigger-generated orphan notifications, caught generically). Cleanup: all delete-class orphans removed; `user_roles` = 3; every valid sibling survived; `reviewed_by` nulled with row kept; both-dead message deleted; **both one-sided messages, both orphaned profiles, and both applications preserved**; idempotent. FK migration: **with orphaned profiles present it failed loud and rolled back (still 2 FKs, nothing partially added)**; after simulating the manual-review resolution it succeeded at **13 FKs / 13 validated with `messages` carrying no auth FK**; CASCADE + SET NULL exercised live on account deletion; future-write enforcement proven (a dead-ref insert is rejected 23503); an existing FK with the **wrong ON DELETE** correctly **raised**; idempotent re-run. `tsc`/`build` clean; lint 0 errors (31 pre-existing warnings); Playwright smoke 11/11 (no frontend changes this pass).
...
1. **Run `scripts/audit_auth_orphans.sql`** (read-only, Q1–Q5 one at a time). Expect: Q1 shows only the WS4/WS8 FKs; Q2 shows `user_roles = 8` and 0 elsewhere; Q5 valid = 3; **Q4 empty**. Review Q3 so every deleted/nulled row is a known quantity. **If Q4 returns any profile row: STOP and resolve it deliberately** — otherwise the FK migration will (by design) fail on push.
2. After merge: `npx supabase db push --linked` (applies `20260713000200` + `20260713000300` only). No Edge Function redeploy; no frontend change ships in this pass.
...
- **Root cause (strong inference from schema, not a directly-verified restore record):** the most consistent explanation is that production was built by `pg_restore` from the Lovable dump with `auth.users` intentionally not migrated, so the 7 declared FKs to `auth.users` (`user_roles`, `student_profiles`, `club_profiles`, `bookmarks`, `messages`×2, `notifications`) could not validate against the fresh `auth.users` and were dropped/skipped. Six further user-ID columns (`notification_preferences`, `waitlist.user_id`, `waitlist.reviewed_by`, `page_views.user_id`, `club_followers`, `reminder_logs`) never had FKs at all. Only the post-repair FKs (`rsvps.status_updated_by` WS4, `club_team_members.user_id` WS8) are enforced in production.
- **Fix:** the Auth-orphan cleanup pass — read-only audit `scripts/audit_auth_orphans.sql`, deterministic cleanup `20260713000200`, FK restoration `20260713000300` (adds the 11 managed FKs as validated constraints; `messages` excluded pending a product decision; **fails loud** rather than leaving an unvalidated constraint if an orphaned profile remains). Full classification + verification in the cleanup record in "▶ Start here".
